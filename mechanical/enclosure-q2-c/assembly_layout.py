...
fpc2_shape=Part.makeBox(3.2,7.9,1.5,V(7.652,14.084,main_z-1.5))
slot=Part.makeBox(2.8,6.9,.28,V(8.15,14.584,3.11))
fpc2=ref('ScreenConnectorFPC2','FPC2 / 主板背面屏幕连接器',fpc2_shape.cut(slot),(.22,.24,.27),'Body XY from existing B.Fab. Height1.5, insertion depth2.5 and slot height are mechanical assumptions; latch/contact face require real part check.')
target=rounded('PCBTargetEnvelope',47,76,10.5,p('MainThickness'),1.5,2,p('MainRearZ'))
ref_native(target,'新板机械空间 / 47 x 76 R10.5, not routed',(.1,.49,.36))
battery=box('BatteryReference',p('BatteryWidth'),p('BatteryLength'),p('BatteryThickness'),p('BatteryX'),p('BatteryY'),p('BatteryZ'))
ref_native(battery,'电池占位 / 主板正面与操作板之间，26 x 34 x 3',(.46,.50,.55))
rf=ref('AntennaKeepoutReference','天线区域 / screen backing still needs RF review',Part.makeBox(22,6.169,6.64,V(1.281,1.431,front_z)),(.85,.25,.12))
...
fpc=ref('DisplayFPCRoute','屏幕 FPC / 正面绕左板边至背面 FPC2',Part.makeCompound(routeparts),(.9,.55,.16),'Centerline route nominal46.14 mm. Side jog is a routing envelope, not a proven developable fold. R0.65/R0.45 and contact slot are assumed. Do not fabricate from this folded ribbon.')
...
fpc.addProperty('App::PropertyString','Connection','Routing').Connection='LCD front -> LEFT edge wrap -> rear service loop -> existing FPC2 (under ESP32)'
