"""Assembly-level checks; keep old PCB conflicts distinct from new mechanical design."""
doc.recompute()
...
pairs += [(control_screws,battery),(control_screws,control),(fpc,fpc2)]
for a,b in pairs:checks['new_physical_interferences_mm3'][a.Name+' / '+b.Name]=round(a.Shape.common(b.Shape).Volume,6)
checks['outside_envelope_mm3']={o.Name:round(o.Shape.cut(Part.makeBox(50,80,14.2)).Volume,6) for o in newphysical+[fpc,controlfpc]}
for source in [doc.MainPCB,doc.MainComponents]:
    for against in [housing,rear,lcd,battery,control,fpc,daughterconn,mainconn,controlfpc]:
        volume=round(source.Shape.common(against.Shape).Volume,6)
        if volume>1e-5:checks['existing_hardware_conflicts'].append({'source':source.Name,'against':against.Name,'volume_mm3':volume,'includes_assumed_heights':source.Name=='MainComponents'})
checks['per_component_conflicts']=[]
for name,shape in component_shapes.items():
    for label,other in [('housing',housing.Shape),('battery',battery.Shape),('control_board',control.Shape),('display_fpc',fpc.Shape),('control_fpc',controlfpc.Shape),('new_main_connector',mainconn.Shape)]:
        ...
        if volume>1e-5:checks['per_component_conflicts'].append({'ref':name,'against':label,'volume_mm3':volume,'height_assumed':True})
...
doc.saveAs(str(HERE/'ESP32S31_MP3_Q2_C.FCStd'))
for obj,stem in [(housing,'q2-c-housing'),(rear,'q2-c-rear-cover'),(wheel,'q2-c-wheel-rocker'),(button,'q2-c-center-button'),(control,'control-board-mechanical')]:
...
Part.makeCompound([o.Shape for o in newphysical+[doc.MainPCB,doc.MainComponents,fpc,controlfpc,fpc2]]).exportStep(str(OUT/'assembly-C-with-current-PCB.step'))
target.Shape.exportStep(str(OUT/'mainboard-target-envelope-NOT-ROUTED.step'))
...
for x,y in control_mounts:out+=['0','CIRCLE','100','AcDbEntity','8','NPTH_HOLES','100','AcDbCircle','10',str(x-3),'20',str(y-44.3),'30','0','40','.9']
out+=['0','ENDSEC','0','EOF'];(OUT/'control-board-outline.dxf').write_text('\n'.join(out)+'\n',encoding='ascii')
rows=['feature,local_x_mm,local_y_mm,side,size_mm,status']
for name,x,y in key_positions:rows.append(f'{name},{x-3:.3f},{y-44.3:.3f},front,3x3x0.55,mechanical keepout only')
for i,(x,y) in enumerate(control_mounts):rows.append(f'Mount{i+1},{x-3:.3f},{y-44.3:.3f},through,D1.8,NPTH mechanical proposal')
rows.append('ControlFPC,37.750,5.700,rear,7.5x3x1,connector and pinout TBD')
(OUT/'control-board-features.csv').write_text('\n'.join(rows)+'\n',encoding='utf8')
print('ASSEMBLY_C_COMPLETE',json.dumps(checks,ensure_ascii=False))
