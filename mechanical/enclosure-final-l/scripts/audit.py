...
phys=cases+electronics+[d.getObject(n) for n in ['LCDModule','CoverLens','ControlFFC40mm','Motor_C08_005','MotorAdhesive','BatteryPack','BatteryInsulation','BatteryAdhesive','MainBoardFasteners','ControlBoardFasteners','RearCoverFasteners']]
phys += [o for o in d.AssemblyReferences.Group if 'Wire' in o.Name]
phys += [d.ControlWashers,d.ControlEdgePad,d.ControlChannelLiner,d.WireEdgeInsulation]
phys += [d.RearInsertTop,d.RearInsertBottom,d.MotorCarrierAdhesive,d.LensAdhesive]
phys=list({o.Name:o for o in phys}.values())
phys += [d.OLEDOpticalBond,d.ScreenRearFPCEnvelope,d.DisplayAdapterPCB,d.DisplayAdapterComponents,d.DisplayAdapterConnectors]
...
for i,a in enumerate(phys):
 ...
 for b in phys[i+1:]:
  ...
  if n in [{'PCB_FPC1','ControlFFC40mm'},{'CTRL_J1','ControlFFC40mm'},{'not-applicable','not-applicable-2'},{'MainBoardFasteners','RearCoverWithPCBPosts'},{'ControlBoardFasteners','HousingControlsA'}]:continue
...
expanded=[]
for o in cases+electronics+[d.ControlFFC40mm,d.Motor_C08_005]+[o for o in phys if o.Name.startswith('MotorWire')]:
 v=volume(o.Shape,d.BatteryExpandedKeepout.Shape)
 if v>1e-5:expanded.append([o.Name,v])
clearances={}
for a,b in [('Motor_C08_005','PCB_CN1'),('Motor_C08_005','ControlsBoard'),('Motor_C08_005','ControlFFC40mm'),('BatteryPack','MainPCB'),('BatteryExpandedKeepout','PCB_U5'),('ControlFFC40mm','RearCoverWithPCBPosts')]:
