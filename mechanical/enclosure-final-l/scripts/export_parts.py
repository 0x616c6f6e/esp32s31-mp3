"""Export independent L parts in millimetres, with no sacrificial frames."""
...
P=O/'prototype-parts-not-released'; P.mkdir(exist_ok=True)
...
objects={o.Name:o for o in d.PrintableParts.Group+d.Electronics.Group+d.AssemblyReferences.Group
 if hasattr(o,'Shape') and not o.Shape.isNull() and o.Name not in ['InsertFitCoupon','LCDActiveArea','LensBlackMask','WheelCopper','ControlSymbols','ScreenRearFPCEnvelope','DisplayAdapterComponents','DisplayAdapterConnectors']}
Import.export(list(objects.values()),str(O/'Q2_L_AM213_Adapter_Study.step'))
