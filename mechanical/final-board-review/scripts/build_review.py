...
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'hardware-controls';OUT=ROOT/'mechanical/final-board-review';OUT.mkdir(exist_ok=True)
V=A.Vector
...
doc=A.newDocument('Q2_Controls_B');doc.Label='Q2 final mainboard + controls / mechanical review 2026-09-22'
parts={};colors={}
...
native=A.newDocument('ControlsImport');Import.insert(str(OUT/'controls-populated.step'),native.Name)
...
exec(compile((OUT/'scripts/full_audit.py').read_text(encoding='utf8'),str(OUT/'scripts/full_audit.py'),'exec'))
...
doc.recompute();doc.saveAs(str(OUT/'Q2_Final_Mainboard_Assembly.FCStd'))
