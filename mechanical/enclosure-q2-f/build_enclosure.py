...
layout=json.loads((ROOT/'hardware-q2-placement/output/placement.json').read_text(encoding='utf8'))
import sys
provenance={'pcb_source_sha256':hashlib.sha256((ROOT/'hardware-q2-placement/hardware.kicad_pcb').read_bytes()).hexdigest()}
assert provenance['pcb_source_sha256']==layout['pcb_sha256'], 'Refresh placement.json after PCB changes.'
mechanical_check=json.loads((ROOT/'hardware-q2-placement/output/mechanical-check.json').read_text(encoding='utf8'))
assert mechanical_check['pcb_sha256']==provenance['pcb_source_sha256']
assert mechanical_check['populated_step_sha256']==hashlib.sha256((ROOT/'hardware-q2-placement/output/placement-populated.step').read_bytes()).hexdigest()
provenance['populated_step_sha256']=mechanical_check['populated_step_sha256']
provenance['harness_document_sha256']=hashlib.sha256((ROOT/'mechanical/connector-layout/Q2_Connector_Layout.FCStd').read_bytes()).hexdigest()
DOCNAME='Q2_Enclosure_F'
if any(n in App.listDocuments() for n in [DOCNAME,'ESP32S31_MP3_Q2_F']):
    raise RuntimeError('The enclosure document is already open. Save/close it before rebuilding.')
doc=App.newDocument(DOCNAME)
doc.Label='ESP32-S31 MP3 / Q2 F / equal screen border / raised control board'
...
outer=rounded('HousingBlank',W,L,p('CornerRadius'),H+'-('+z0+')',y=p('ShellTopY'),z=z0)
doc.recompute()
...
cavity=rounded('HousingCavity',W+'-2*Parameters.Wall',L+'-2*Parameters.Wall','Parameters.CornerRadius-Parameters.Wall',H+'-Parameters.FrontThickness+1','Parameters.Wall','Parameters.ShellTopY+Parameters.Wall',-1)
...
# Top extension leaves the original screw axis fixed to the PCB notch.
# A high bridge joins its boss to the new wall, above the PCB/ESP32 envelope.
bridge=box('TopScrewBossBridge',3,3.5,1,23.5,'Parameters.ShellTopY+Parameters.Wall-0.1',9.6)
housing=fuse('HousingWithTopBossBridge',[housing,bridge])
...
orient(jack,App.Rotation(App.Vector(1,0,0),90),36.435,81,7.9)
...
rear=rounded('RearBlank',W,L,p('CornerRadius'),p('RearThickness'),y=p('ShellTopY'))
lipout=rounded('RearLipOuter',47.2,'Parameters.Length-2.8','Parameters.CornerRadius-1.4',1.2,1.4,'Parameters.ShellTopY+1.4',1)
lipin=rounded('RearLipInner',45.6,'Parameters.Length-4.4','Parameters.CornerRadius-2.2',1.6,2.2,'Parameters.ShellTopY+2.2',.8)
...
actuators=[cylinder('WheelActuator'+str(i),.85,.41,x,f'{y}+Parameters.ControlShiftY',12.55) for i,(x,y) in enumerate([(25,48.3),(12,61.3),(38,61.3),(25,74.3)])]
...
def stroke(points,width=.16):
    points=[(50-x,y+params['ControlShiftY']) for x,y in points]
    ss=[]
