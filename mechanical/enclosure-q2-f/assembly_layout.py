...
# Use the current populated STEP, including the new H1/H2 ACH connectors.
import Import
native=App.newDocument('F_PCB_Import');Import.insert(str(ROOT/'hardware-q2-placement/output/placement-populated.step'),native.Name)
assembly=next(o for o in native.Objects if o.Label=='placement-populated 1')
fps={f['reference']:f for f in layout['footprints'] if not f['reference'].startswith('SCREW')}
component_shapes={};board_shape=None
for item in assembly.OutList:
    if not hasattr(item,'Shape') or item.Shape.isNull() or item.Shape.Volume<1e-8:continue
    origin=item.Placement.Base
    name=min(fps,key=lambda r:math.hypot(fps[r]['xy'][0]-origin.x,fps[r]['xy'][1]+origin.y))
    sh=item.Shape.copy();sh.rotate(V(),V(0,0,1),180)
    if math.hypot(fps[name]['xy'][0]-origin.x,fps[name]['xy'][1]+origin.y)>.005:
        sh.translate(V(149,-49,-sh.BoundBox.ZMin));mat=App.Matrix();mat.A33=params['MainThickness']/sh.BoundBox.ZLength
        sh=sh.transformGeometry(mat);sh.translate(V(0,0,main_z));board_shape=sh
    else:
        sh.translate(V(149,-49,(front_z if fps[name]['side']=='F' else main_z)-origin.z));component_shapes[name]=sh
assert set(component_shapes)==set(fps) and board_shape is not None
App.closeDocument(native.Name);App.setActiveDocument(doc.Name)
ref('MainPCB','46 x 76 main PCB / current unrouted revision',board_shape,(.12,.40,.27))
ref('MainComponents','122 current components / actual project STEP models',Part.makeCompound(list(component_shapes.values())),(.42,.45,.49))
esp=ref('ESP32Front','ESP32 front / toward screen',component_shapes['U9'].copy(),(.70,.73,.75))
fpc2=ref('ScreenConnectorFPC2','FPC2 rear / insertion still unverified',component_shapes['FPC2'].copy(),(.70,.22,.18))
# Keep the latest motor and wiring plan in this complete exterior assembly.
oldplan=App.openDocument(str(ROOT/'mechanical/connector-layout/Q2_Connector_Layout.FCStd'))
harness_parts=[]
for name in ['Motor_C08_005','MotorHolderProposal','MotorAdhesive','MotorWire1','MotorWire2','BatteryWire1','BatteryWire2','BatteryWire3']:
    obj=oldplan.getObject(name);harness_parts.append(ref(name,obj.Label,obj.Shape.copy(),(.85,.55,.20),obj.PlanningNote))
App.closeDocument(oldplan.Name);App.setActiveDocument(doc.Name)
motor_max=Part.makeCylinder(4.05,3.45,V(39,57,7.1))
...
control_mounts=[(x,y+params['ControlShiftY']) for x,y in [(6,53),(44,53),(6,69),(44,69)]]
...
control.addProperty('App::PropertyString','LocalDatum','PCB').LocalDatum='Local origin is case (3,42.3). Hole centers (3,8.7),(41,8.7),(3,24.7),(41,24.7); diameter1.8.'
...
key_positions=[(name,x,y+params['ControlShiftY']) for name,x,y in key_positions]
switches=[]
...
electrodes=[]
for i in range(12):
    shape=Part.makeCylinder(15.5,.012,V(25,params['WheelY'],control_top),V(0,0,1),26).cut(Part.makeCylinder(10.5,.03,V(25,params['WheelY'],control_top-.01)))
    shape.rotate(V(25,params['WheelY'],0),V(0,0,1),i*30+2);electrodes.append(shape)
...
daughterconn=ref('ControlFPCConnector','操作板背面 FPC 座预留 / pin count TBD',Part.makeBox(7.5,3,1,V(37,48.5+params['ControlShiftY'],10.1)),(.14,.16,.19),'7.5 x 3 x 1 mm placeholder, not a chosen part or pinout.')
...
controlpath=[V(40.75,50.0+params['ControlShiftY'],10.55),V(40.75,43.6+params['ControlShiftY'],10.55),V(40.75,43.6+params['ControlShiftY'],8.0),V(40.75,35.5,8.0),V(40.75,35.5,6.0)]
ctrlsh=[]
...
mainmounts=[(149-f['xy'][0],f['xy'][1]-49) for f in layout['footprints'] if f['reference'].startswith('SCREW')]
