...
res['controls_before_motor']=stage('CONTROLS',control,[d.HousingControlsA,d.WheelControlsA,d.CenterButtonControlsA,d.ControlEdgePad],
 [[-1.3,-5,z] for z in [-18,-14,-10,-8,-6,-4,-2,-1,-.5]]+ [[-1.3,y,-.5] for y in [-4,-3,-2,-1,0]]+ [[-1.3,0,z] for z in [-.3,0]]+[[x,0,0] for x in [-.9,-.6,-.3,0]])
...
res['lcd_from_front']=stage('LCD',[d.LCDModule],[d.HousingControlsA,d.LCDMountTape],[[0,0,z] for z in [10,6,3,1,.5,.2,0]])
if '--rear' in sys.argv:
 ...
 fixed=[d.HousingControlsA,d.ControlsBoard,d.WheelControlsA,d.CenterButtonControlsA,d.MotorCarrier,d.Motor_C08_005,d.LCDModule,d.RearInsertTop,d.RearInsertBottom,d.ControlBoardFasteners,d.ControlWashers,d.ControlEdgePad,d.BatteryPack]+[o for o in d.Electronics.Group if o.Name.startswith('CTRL_')]
 res['rear_mainboard_closing']=stage('REAR',main,fixed,[[0,0,z] for z in [-15,-10,-5,-2,-1,-.5,0]])
res['method']='Staged rigid poses; motor fitted after controls, LCD from front before lens, rear PCB last; cables not rigidly translated. This is not continuous swept-volume, tooling or flex simulation.'
