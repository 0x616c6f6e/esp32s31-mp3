...
def build():
 ...
 edge=3.275;tx=2.05;top=13.0;run=2.05;ret=3.35;tip=8.352
 prefix=edge-tx+math.pi*r+(top-r-(run+r));neck=13.8-prefix;assert neck>0
 nx=tx+neck;rise=.35;theta=math.acos(1-rise/(2*r));dx=2*r*math.sin(theta)
 ramp_end=11.5;ramp_start=ramp_end+dx
 # First 13.8 mm is the wide tail neck; then a 6.6 mm uniform ribbon.
 constant=13.8-nx+math.pi*r-ramp_start+2*r*theta+ramp_end-tip
