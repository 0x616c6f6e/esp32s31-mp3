...
for e in json.loads((O/'export-check.json').read_text()):
 p=O/'print-package'/e['part']; s=Part.read(str(p.with_suffix('.step'))); m=Mesh.Mesh(str(p.with_suffix('.stl')))
...
report['assembly_sha256']=hashlib.sha256((O/'Q2_K_Assembly_Access.FCStd').read_bytes()).hexdigest()
