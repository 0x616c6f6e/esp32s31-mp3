...
DOCNAME='LCD209_Dimensioned'
if any(n in App.listDocuments() for n in [DOCNAME,'LCD209_36x43']):
...
terminals=[]
for i in range(21):
    terminals.append(box('Contact%02d'%(i+1),.18,p('FPCContactLength'),.015,f'(Parameters.LCDWidth-Parameters.FPCTerminalSpan)/2+{i}*Parameters.FPCPitch-0.09','Parameters.LCDLength+Parameters.FPCExtension-Parameters.FPCContactLength','0.25+Parameters.FPCThicknessEstimated'))
...
fpc.addProperty('App::PropertyString','DimensionalProvenance','Source').DimensionalProvenance='User drawing: length 46.14, tail width 6.6, terminal exposure 2.5, center span 6 and pitch0.3. Neck 18x12.8, t0.12, pad width0.18, copper thickness0.015 estimated. Pin1 handedness/contact face must be checked.'
...
doc.saveAs(str(HERE/'LCD209_36x43.FCStd'))
