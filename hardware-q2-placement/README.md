...
已调整 76 个元件的位置、角度或板面；122 个实体元件分为正面 52 个、背面 70 个，另有 4 个固定安装孔。板框仍为 46 × 76 mm、R10、六层。
...
U9、CARD2、CN1、USB1、FPC2、H1、H2 以及 4 个安装孔保持原坐标/板面，并已锁定，以维持既有外壳接口和排线关系。需要调整时可在 KiCad 解锁。天线禁布区保留；屏幕 FPC 仍通过板边绕到背面连接座。
...
已移除旧 2903 条直线段、48 条弧线、340 个过孔和 21 个铺铜区；**未新增任何走线**。保留全部元件、555 个焊盘及网络、原理图、六层设置、原设计规则、板框/孔槽、天线禁布区和本地 3D 模型。旧的局部 SW 铜区也已移除，避免位置变化后留下错误连接。
...
- 元件/焊盘网络逐项与源板核对：126 个封装、555 个焊盘，网络变化 0；原理图、板框、天线禁布区和机械锚点保持一致。
...
`scripts/place_components.py` 会从原板重建本方案并覆盖本版 PCB；开始手动修改或布线后不要重跑。验证脚本和预览脚本可按需使用，源资料仍需保留在此仓库原路径。旧版检查报告和外壳占位不能用来代表你后续手动修改后的状态。
