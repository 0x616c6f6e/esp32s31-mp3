...
sys.path.insert(0,str(ROOT/'tmp'));from sexpr import parse,children,get,dump
...
def padmap(b):return {(f.GetReference(),a.GetNumber(),a.m_Uuid.AsString()):a.GetNetname() for f in b.GetFootprints() for a in f.Pads()}
...
for f in (P).glob('*.kicad_sch'):assert f.read_bytes()==(ROOT/'hardware-q2'/f.name).read_bytes()
